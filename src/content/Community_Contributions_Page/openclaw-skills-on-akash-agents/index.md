...
description: What they are, how they work, and how to build your own — deploying OpenClaw AI agents on Akash Network and building custom skills to automate your workflows through Telegram or Discord.
pubDate: 2026-06-04
draft: false
categories:
  - Guides
...
OpenClaw is an open-source AI agent that takes action on your behalf. This includes reading files, running commands, browsing the web, and managing your calendar, all through messaging apps like Telegram or Discord. Think of it as a personal assistant that works around the clock.

AkashAgents gives you a one-click OpenClaw deployment (among other Agent Platforms) on the Akash Network: decentralised compute, no lock-in, a few dollars a month, ability to teardown and redeploy anytime. But deploying OpenClaw is just the beginning. What makes it genuinely useful over time are skills, small instruction files that teach the agent how to do specific jobs the way you want them done.
...
There are over 60,000 community-built skills on ClawHub, OpenClaw's public registry. That sounds convenient. It's also a real risk.
...
- CONSOLE API KEY: your Akash Console API key (prefixed akash_console_...). Generate one at console.akash.network/user/api-keys via the GET KEY link.
- AKASHML API KEY : your AkashML API key (prefixed sk-akashml-...). Generate one at akashml.com via the GET KEY link.
...
Click DEPLOY AGENT NOW. The platform will confirm when your bot is ready with a 'Your Telegram Bot is ready to chat' message and a direct link to your bot. Note: Deployments on trial accounts are automatically closed after 24 hours, resulting in the loss of any data your agent may have learnt, so make sure to top up US$20 or more to activate your account, remove this restriction and prevent your free trial funds from expiring after the month is finished.

### Managing Your Deployment

After deploying, manage your running agent from console.akash.network. Find your deployment by its DSEQ number (shown on the post-deploy screen). The deployment detail page provides six tabs: Leases, Alerts, Logs, Shell, Events and Update.
...
Every Akash OpenClaw deployment serves the OpenClaw Control UI at your deployment's public URL. Find it in the Akash Console under your deployment details. Open it in a browser, connect following the instructions and you get a full dashboard: chat, skills status, config editor, cron jobs, and live logs, no shell access required for most tasks. Here, I have a weekend skill bot deployed successfully.

![](./images/image1.png)

![](./images/image2.png)

> **PORT MAPPING:** The Akash SDL maps the OpenClaw Gateway port (18789) to public port 80. Your deployment URL accesses the Control UI without any port suffix, just the plain URL shown in Akash Console. Do not append:18789 to the public URL.
...
> **VERIFY STORAGE BEFORE WRITING SKILLS:** The Akash OpenClaw SDL includes a 10G storage volume. Before writing any skills or config, confirm this volume is mounted correctly. Open the Shell tab and run:
...
### Where Skills Live on Akash OpenClaw

On an Akash OpenClaw deployment, skills go in the container directory /data/.openclaw/.openclaw/workspace/<yourskill>.md. Skills placed there are available across all sessions, as long as your storage volume is correctly mounted (see below).
...
---
 ...
 description: One-line using words users actually type. Use when the user asks about
   [specific phrasing]. Handles [specific tasks].
 user-invocable: true
...
# Find skill files:
 ...
 cat /data/openclaw/.openclaw/workspace/weekend-roundup/
```
...
| **Resource** | **URL** |
| --- | --- |
| Deploy OpenClaw on Akash Agents | agents.akash.network |
| Get Console API Key | console.akash.network/user/api-keys |
| Get AkashML API Key | akashml.com |
| Get Telegram Bot Token | @BotFather on Telegram |
| OpenClaw Docs | docs.openclaw.ai |
| OpenClaw GitHub | github.com/openclaw/openclaw |
| ClawHub (skill registry) | clawhub.com |
| Akash Console (manage deployments) | console.akash.network |
| Akash Developer Docs | akash.network/docs/developers |

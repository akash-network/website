...
tags: ["Network Upgrades", "Mainnet 14"]
weight: 1
title: "Akash v1.0.0 Node Upgrade Guide"
linkTitle: "Mainnet 14 Upgrade"
---
...
If you downloaded a precompiled binary:
...
If you built from source (see [Build Binary From Sources](#build-binary-from-sources)), the binary will be in `.cache/goreleaser/main/<YOUR_OS_ARCH>/akash`:

```bash
mkdir -p $HOME/.akash/cosmovisor/upgrades/v1.0.0/bin

# Copy from build output (replace <YOUR_OS_ARCH> with your platform)
...
After build completes, binaries will be located in `.cache/goreleaser/main/`. Select the directory matching your operating system and architecture (e.g., `linux_amd64_v1/`, `darwin_arm64/`, etc.).

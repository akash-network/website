...
If set correctly, then the command below should return the name you've just defined.

```bash
echo $AKASH_KEY_NAME
```

Next, we'll set the `$AKASH_KEYRING_BACKEND` environment variable to `os`. This will configure `provider-services` to store all of our keys using our operating system's native key manager (e.g. MacOS Keychain, Windows Credentials Management API).

```bash
AKASH_KEYRING_BACKEND=os
# Confirm that AKASH_KEYRING_BACKEND is set to os
echo $AKASH_KEYRING_BACKEND
```

We're finally ready to create our first key! To do so, we'll run the following command:

```bash
provider-services keys add $AKASH_KEY_NAME
```

If everything works as expected, the command above will return information about your newly created key. **Do not continue on without first saving your mnemonic phrase in a safe place**. If you need to recover your account at a later time, or wish to add your account to a browser extension, then you must have this phrase accessibile. If someone manages to access this phrase, then they will have control over the account and any assets held within it, so you should never send this phrase to anyone. 

No one associated with Akash will ever request this information from you. If at any point someone requests that you share your phrase with them, then you are advised to cease all communications with the person and report this instance to the Akash team.

Next, let's create a shell variable in your terminal named `AKASH_ACCOUNT_ADDRESS` and set it to be the wallet address `provider-services` generated at the time of account creations.
...
Note that if you close your terminal window this variable will not be saved. For those who wish to have this variable persist throughout restarts of your shell, then you can add the `export ...` line to your terminal's profile file (e.g. `.bashrc`, `.zshrc`). 

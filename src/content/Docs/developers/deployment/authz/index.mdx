...
- **`/akash.deployment.v1beta4.MsgUpdateDeployment`** - Update existing deployments
- **`/akash.deployment.v1beta4.MsgCloseDeployment`** - Close deployments
- **`/akash.market.v1beta5.MsgCreateLease`** - Create leases with providers
- **`/akash.market.v1beta5.MsgWithdrawLease`** - Withdraw from leases
...
```bash
provider-services tx feegrant grant <grantee-address> \
  --from <your-wallet> \
  --fees 5000uakt
```
...
See **[AuthZ & Fee Grants SDK](/docs/api-documentation/sdk/authz-feegrant)** for programmatic usage with Go and JavaScript/TypeScript.
...
```bash
# Bad: Unlimited fees
provider-services tx feegrant grant <grantee> --from <wallet>

# Good: Limited fees
provider-services tx feegrant grant <grantee> \
...
provider-services tx feegrant grant <bot-address> \
  --period 86400 \
...
provider-services tx feegrant grant <new-member> \
  --spend-limit 10000000uakt \
...
provider-services tx feegrant grant <contractor> \
  --spend-limit 1000000uakt \
...
# First generate the tx JSON as the granter:
provider-services tx deployment create deploy.yaml --from <granter> --generate-only > tx.json
# Then execute it as the grantee:
provider-services tx authz exec tx.json \
  --from <grantee-wallet> \
  --fees 5000uakt
```
...
**→ [AuthZ & Fee Grants SDK Documentation](/docs/api-documentation/sdk/authz-feegrant)**
...
- **[CLI Reference](/docs/developers/deployment/cli)** - Complete CLI commands
- **[SDK Documentation](/docs/api-documentation/sdk)** - Programmatic AuthZ usage

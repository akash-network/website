...
Refer to the [ Akash Provider](/docs/docs/providers/build-a-cloud-provider/akash-cloud-provider-build-with-helm-charts/) guide if your provider has not yet been built.
...
Install Helm and add the Akash repo if not done previously by following the steps in this [guide](/docs/docs/providers/build-a-cloud-provider/akash-cloud-provider-build-with-helm-charts#step-4---helm-installation-on-kubernetes-node)**.**
...
- Ensure that necessary [environment variables](/docs/docs/providers/build-a-cloud-provider/akash-cloud-provider-build-with-helm-charts#step-6---provider-build-via-helm-chart) are in place prior to issuing
...
- _**NOTE**_ - the `bidpricestoragescale` setting in the `provider.yaml` file will be ignored if the [bid pricing script](/docs/docs/providers/build-a-cloud-provider/akash-provider-bid-pricing-calculation/) is used.

...
> _**NOTE**_ - using the procedures in this guide the Validator private key will be stored within the associated Akash deployment. Ensure that only known, trusted providers are used for such deployments. Additionally consider storing the private key outside of the deployment through the use of TMKMS which is documented [here](/docs/for-node-operators/validators/tmkms-stunnel/).
...
\
If you have not used Akash Console prior and/or are not overly familiar with the app, please review our [Akash Console documentation](/docs/for-developers/akash-console/) before getting started with the Validator build steps detailed in subsequent sections.

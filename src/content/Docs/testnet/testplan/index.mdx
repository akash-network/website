...
### [Category 11: Security Testing](/docs/testnet/testplan/security)
Validate cryptographic protections and access control enforcement across the BME oracle price pipeline and token transfer restrictions.
- ACT token transfer restriction (bank send disabled between accounts)
- VAA tampering rejection — modified price in existing VAA
- VAA tampering rejection — completely fabricated VAA
- Unauthorized oracle source rejection
- Replay attack rejection — stale VAA resubmission
...
| Security Testing | 11.1-11.5 | High | testnet-oracle (11.2-11.5); next build (11.1) |

**Total: 57 tests across 12 categories**
...
8. **ACT transfers disabled** - Bank sends of `uact` between accounts are blocked; ACT enters and exits only via BME mint/burn operations